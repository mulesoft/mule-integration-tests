...
public class MuleTestNamespaceFunctionalTestCase extends AbstractIntegrationTestCase {
  ...
  public void testService1() throws Exception {
    InternalEvent event = flowRunner("testService1").withPayload("foo").run();
  ...
  public void testService2() throws Exception {
    InternalEvent event = flowRunner("testService2").withPayload("foo").run();
    Message message = event.getMessage();
    assertNotNull(message);
    assertThat(event.getError().isPresent(), is(false));
  ...
  public void testService3() throws Exception {
    InternalEvent event = flowRunner("testService3").withPayload("foo").run();
    Message message = event.getMessage();
    assertNotNull(message);
    assertThat(event.getError().isPresent(), is(false));
