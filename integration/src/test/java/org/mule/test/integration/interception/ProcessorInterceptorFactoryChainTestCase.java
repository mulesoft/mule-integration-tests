...
public class ProcessorInterceptorFactoryChainTestCase extends AbstractIntegrationTestCase {
  ...
  //
  // assertThat(killInterceptionParameter.getParameters().keySet(), containsInAnyOrder("targetValue", "victim",
  // "goodbyeMessage"));
  ...
  //
  // @Test
  // @Description("Verify that even if an operation has parameters with invalid expressions, before is called for the
  // interceptor.")
  ...
  //
  // assertThat(moduleOperationChain.getParameters().keySet(), containsInAnyOrder("doc:name", "nameSetPayload", "targetValue"));
  ...
  //
  // @Test
  // @Description("Processors in global error handlers are intercepted correctly for an unknown status code errors in http
  // request")
  ...
  @Test
  public void operationWithChainFailing() throws Exception {
